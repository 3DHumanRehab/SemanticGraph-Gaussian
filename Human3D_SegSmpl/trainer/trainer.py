...
class InstanceSegmentation(pl.LightningModule):
    ...
    def save_visualizations2(
        ...
        if export_files:
            ...
            import trainer.SegmentDataUtils as seg_utils
            print(f'\n{file_name}:')
            print(set(part_semseg))
            plydata = seg_utils.generate_plydata_with_Points_Colors_SegLabels(points=full_res_coords, colors=pred_output, seg_labels=part_semseg)
            seg_utils.save_plyfile(plydata,f"{self.config['general']['save_dir']}/export/{file_name}/SegHuman.ply")
